...
var accountsMerge = function(accounts) {
    const accountsUF = new UnionFind(accounts.length);
    const emailMap = {};
    ...
    for (let email in emailMap) {
        const array = emailMap[email];
        for (let i = 1; i < array.length; i++) {
            accountsUF.union(array[i], array[i - 1]);
        }
    }
    ...
    for (let i = 0; i < accounts.length; i++) {
        ...
        const root = accountsUF.root(i);

        if (!result[root]) result[root] = {};

        for (let j = 1; j < account.length; j++) {
            result[root][account[j]] = true;
        }
    }

    return Object.keys(result).map(index => {
        const name = accounts[index][0];
        const emails = Object.keys(result[index]).sort();
        return [name, ...emails];
    });
};
